...
int	surrounded_vertical(int y, char **map)
{
	...
	while (map[0][i] != '\0' && map[y - 1][i] != '\0')
	{
		...
		{
			ft_printf("Invalid map, must be surrounded by 1\n");
			return (1);
		}
		i++;
	}
...
int	surrounded_horizontal(int x, int y, char **map)
{
	...
	while (j < y && *map[j] != '\0')
	{
		...
		{
			ft_printf("Invalid map, must be rectangle\n");
			return (1);
		}
...
int	ft_condition(t_check_map check)
{
	...
	{
		ft_printf("Invalid map !! (Check P or E or C)\n");
		return (1);
	}
	if (check.e_count > 1 || check.p_count > 1)
	{
		ft_printf("Invalid map !! (Check P or E)\n");
		return (1);
	}
	return (0);
}
